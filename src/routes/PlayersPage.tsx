...
export default function PlayersPage() {
  ...
  useEffect(() => {
    getUsersTop(100).then(setUsers).finally(() => setLoading(false));
  }, []);
        ...
        {users.map(u => (
          <li key={u.id} className="py-3 flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm opacity-70">Сыграно игр: {u.gamesPlayed ?? 0}</div>
            </div>
            <Link to={`/players/${u.id}`} className="underline">Профиль</Link>
          </li>
        ))}
      </ul>
    </div>
